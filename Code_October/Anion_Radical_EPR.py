...
gammaL = 0.175004e-4        # Linewidth (Lorentzian, Tesla)
...
# number of different nuclei with different equiv. nuclei
...
for m1 in M_I_1:
    count2 = 0
    for m2 in M_I_2:
        total_mI = A1_Hz * m1 + A2_Hz * m2
        B_res = (h * freq - total_mI * h) / (g_e * mu_B)
        intensity = intensities_1[count1] * intensities_2[count2]
        resonance_fields.append(B_res)
...
# is usually used. Thus we find the numerical derivative of the spectrum wrt. B0. B0[1] - B0[0]
# subsequently determines our step-size.
deriv_spectrum = np.gradient(spectrum, B0[1] - B0[0])


# Plot
#plt.plot(B0 * 1e3, spectrum/np.max(np.abs(spectrum)), c='r', label='0th derivative')
plt.plot(B0 * 1e3, deriv_spectrum / np.max(np.abs(deriv_spectrum)), c='black', label='1st derivative')
plt.xlabel("Magnetic Field [mT]")
plt.ylabel("Intensity [a.u.]")
plt.legend()
#plt.title("Simulated CW-EPR Spectrum (S=1/2, I=1/2)")
plt.xlim(336.75, 340.5)
plt.grid()
plt.show()
